...
import re
import logging
from content_moderator_agent.tools import (
    generate_text,
    check_grammar,
    get_usage_stats,
)
...
def before_agent_callback(callback_context: CallbackContext) -> Optional[types.Content]:
    ...
    """
    logger.info(f"[AGENT START] [{callback_context.agent_name}] [Invocation: {callback_context.invocation_id}]")

    # Check if agent is in maintenance mode
    if callback_context.state.get('maintenance_mode', False):
...
def after_agent_callback(callback_context: CallbackContext) -> Optional[types.Content]:
    ...
    """
    logger.info(f"[AGENT COMPLETE] [{callback_context.agent_name}] [Invocation: {callback_context.invocation_id}]")

    # Track successful completions
    callback_context.state['agent_completed'] = True

    # We could add standard disclaimer here: This is AI-generated content
...
def before_model_callback(
    ...
    if llm_request.contents:
        # Get the last content part from the request
        last_content = llm_request.contents[-1]
        for part in last_content.parts:
            if part.text:
                user_text += part.text

    logger.info(f"[LLM REQUEST] [{callback_context.agent_name}] [Invocation: {callback_context.invocation_id}] Length: {len(user_text)} chars")

    # GUARDRAIL: Check for blocked words
    for word in BLOCKED_WORDS:
        if word.lower() in user_text.lower():
            logger.warning(f"[LLM BLOCKED] [{callback_context.agent_name}] [Invocation: {callback_context.invocation_id}] Found blocked word: {word}")
...
def after_model_callback(
    ...
    logger.info(f"[LLM RESPONSE] [{callback_context.agent_name}] [Invocation: {callback_context.invocation_id}] Length: {len(response_text)} chars")
...
def after_tool_callback(
    ...
    tool_context.state['last_tool_result'] = str(tool_response)
